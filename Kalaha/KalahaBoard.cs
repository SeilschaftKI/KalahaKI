...
namespace Kalaha
{
	...
	{	
		...
		{	
			for (int i = 0; i < Slots.GetLength(0); i++) { //erstmal werden alle felder auf 0 gesetzt
				Slots[i]=0;
			}
...
//		}
		...
		public bool move(int moveChoice, int StartSide) //Funktion gibt true zurueck wenn man im eigenen kalaha-feld landet (dann darf man nommal)
		{
			...
			Console.WriteLine("move("+(1+moveChoice)+","+StartSide+") wird jetzt ausgeführt... Ergebnis:");
		...
		public bool SideEmpty(int Side)
		{
			...
			for (int i =Side*TotalSlots/2; i < (1+Side)*TotalSlots/2-1; i++) {
				if (Slots[i]!=0) {
					alert = true;
					break;
				}
		...
		{
			for (int i =Side*TotalSlots/2; i < (1+Side)*TotalSlots/2-1; i++) {//UNDER CONSTRUCTION! TODO
//				Slots[indKalahaSlot[Change0and1(Side)]]+=Slots[i]; ALT
				Slots[indKalahaSlot[(Side)]]+=Slots[i];
				Slots[i] = 0;
			}
		}
